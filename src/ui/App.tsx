...
import type { TaskPriority, TaskStatus, ChannelHealth, DispatchTask } from "../channels/unified-dispatch.js";
...
const VALID_MODES: readonly WotannMode[] = [
  "default", "plan", "acceptEdits", "auto", "bypass", "autonomous", "guardrails-off",
...
export function WotannApp({
  ...
  const [stats, setStats] = useState({
    cost: 0, contextPercent: 0, reads: 0, edits: 0, bashCalls: 0,
  });
  ...
  const handleMessageAction = useCallback((action: MessageAction, messageId: string) => {
    ...
    switch (action) {
      case "copy":
        try { execFileSync("pbcopy", [], { input: targetMsg.content, timeout: 2000 }); } catch { /* clipboard unavailable */ }
    ...
    switch (cmd) {
      ...
      case "/providers": {
        const lines = providers.map((p) => {
          ...
          const models = p.available && p.models.length > 0
      ...
      case "/skills": {
        ...
        if (arg) {
          ...
          const matches = summaries.filter((s) =>
            s.name.includes(q) || s.description.toLowerCase().includes(q) || s.category.includes(q),
          );
      ...
      case "/model": {
        ...
        if (!arg) {
          const modelLines = [...providerModels.entries()].map(([prov, models]) =>
            `  ${prov}: ${models.join(", ")}`,
          );
          sysMsg([
      ...
      case "/doctor": {
        ...
        const registry = getSkillRegistry();
        const runtimeStatus = runtime ? "active (full pipeline)" : "not initialized (raw bridge mode)";
        sysMsg([
          "WOTANN Health Check",
          "",
          ...
          runtime ? `  Memory:      ${runtime.getStatus().memoryEnabled ? "SQLite + FTS5" : "disabled"}` : "",
          "",
          !runtime ? "  ⚠ Runtime not active — harness intelligence disabled. Run wotann init." : "  ✓ Full harness intelligence active",
        ].filter(Boolean).join("\n"));
        return true;
      }

      case "/compact": {
        if (!runtime) { sysMsg("Compact requires WotannRuntime."); return true; }
        const budget = runtime.getContextBudget();
      ...
      case "/theme": {
        ...
        if (!arg) {
          sysMsg([
            ...
            themeManager.getThemeNames().slice(0, 16).map((name) => `  ${name}`).join("\n"),
      ...
      case "/voice": {
        ...
        if (subcommand === "say" && remainder) {
          void (async () => {
            ...
            sysMsg(spoken ? "Voice playback started." : "Voice playback unavailable. Run `/voice on` and configure TTS.");
          })();
          return true;
        }
      ...
      case "/freeze": {
        if (!runtime) { sysMsg("File freezing requires WotannRuntime."); return true; }
      ...
      case "/branch": {
        if (!runtime) { sysMsg("Branching requires WotannRuntime."); return true; }
        const branchMgr = runtime.getBranchManager();
      ...
      case "/canvas": {
        if (!runtime) { sysMsg("Canvas requires WotannRuntime."); return true; }
      ...
      case "/autonomous": {
        if (!runtime) { sysMsg("Autonomous mode requires WotannRuntime."); return true; }
        if (!arg) {
            ...
            const result = await executor.execute(
              ...
              async () => {
                const tsc = await new Promise<boolean>((resolve) => {
                  try { execFileSync("npx", ["tsc", "--noEmit"], { cwd: runtime.getWorkingDir(), timeout: 60000 }); resolve(true); } catch { resolve(false); }
                });
                const tests = await new Promise<boolean>((resolve) => {
                  try { execFileSync("npx", ["vitest", "run", "--reporter=dot"], { cwd: runtime.getWorkingDir(), timeout: 120000 }); resolve(true); } catch { resolve(false); }
                });
                return { testsPass: tests, typecheckPass: tsc, lintPass: true, output: tsc && tests ? "All checks pass" : "Checks failed" };
              },
              {
      ...
      case "/autoresearch": {
        if (!runtime) { sysMsg("Autoresearch requires WotannRuntime."); return true; }
      ...
      case "/replay": {
        ...
        sysMsg(events.length > 0
          ? `Session recording: ${events.length} events\n${events.slice(-10).map((e) => `  ${new Date(e.timestamp).toISOString().slice(11, 19)} — ${e.type}`).join("\n")}`
          : "No events recorded yet.");
        return true;
      }

      case "/arena": {
        if (!runtime) { sysMsg("Arena requires WotannRuntime."); return true; }
      ...
      case "/council": {
        if (!runtime) { sysMsg("Council requires WotannRuntime."); return true; }
        ...
        void (async () => {
          try {
            ...
            const memberSummary = result.members.map((m) =>
            ...
            const ranking = result.aggregateRanking.length > 0
              ? "\n\nRanking:\n" + result.aggregateRanking.map((r, i) =>
                `  ${i + 1}. ${r.label} (avg rank: ${r.averageRank.toFixed(1)})`,
              ).join("\n")
              : "";
      ...
      case "/inspect": {
        if (!runtime) { sysMsg("Inspect requires WotannRuntime."); return true; }
        const inspector = runtime.getContextInspector();
        if (!inspector) { sysMsg("Context inspector not available."); return true; }
        const snapshot = inspector.getSnapshot();
        const topConsumers = snapshot.topConsumers
          .slice(0, 8)
          .map((s: { source: string; tokens: number; percent: number }) =>
        ...
        sysMsg([
          ...
          ...snapshot.recommendations.slice(0, 3).map((r: string) => `│    • ${r.slice(0, 52)}`),
          "╰──────────────────────────────────────────────────────────╯",
        ].join("\n"));
        return true;
      }
        ...
        void (async () => {
          ...
          switch (mcpAction) {
            ...
            case "install": {
              ...
              if (!serverName) { appendSystemMessage("Usage: /mcp install <name>"); break; }
              appendSystemMessage(`Installing MCP server: ${serverName}\nUse /mcp list to see registered servers.`);
              break;
            }
            case "remove": {
              const serverName = sub[1];
              if (!serverName) { appendSystemMessage("Usage: /mcp remove <name>"); break; }
            ...
            case "audit": {
              const serverName = sub[1];
              if (!serverName) { appendSystemMessage("Usage: /mcp audit <name>"); break; }
      ...
      case "/persona": {
        if (!runtime) { sysMsg("Persona requires WotannRuntime."); return true; }
      ...
      case "/permission": {
        ...
        if (runtime) { runtime.setPermissionMode(permMode as "auto-approve" | "ask-always" | "smart"); }
        sysMsg(`Permission mode set to: ${permMode}`);
        return true;
      }

      case "/healing": {
        if (!runtime) { sysMsg("Self-healing requires WotannRuntime."); return true; }
        ...
        type HealingResult = { category: string; confidence: number; suggestedFix?: string; file?: string; line?: number; relatedPatterns: readonly string[] };
        const pipeline = runtime.getSelfHealingPipeline() as unknown as { classifyError?: (e: string) => HealingResult };
        const classifyFn = pipeline.classifyError ?? ((_: string): HealingResult => ({ category: "unknown", confidence: 0.5, relatedPatterns: [] }));
        ...
        sysMsg([
          ...
          classified.file ? `  File:        ${classified.file}${classified.line ? `:${classified.line}` : ""}` : "",
      ...
      case "/lsp": {
        ...
        switch (lspAction) {
          ...
          case "find": {
            const sym = lspSub[1];
            if (!sym) { sysMsg("Usage: /lsp find <symbol>"); break; }
            if (runtime) {
              void (async () => {
                const lsp = runtime.getLspManager() as unknown as { findSymbol(n: string): Promise<readonly { kind: string; name: string; uri: string; range: { start: { line: number } } }[]> };
                const results = await lsp.findSymbol(sym);
                appendSystemMessage(results.length > 0
                  ? `Found "${sym}" (${results.length} matches):\n${results.slice(0, 10).map((r) => `  ${r.kind} ${r.name} — ${r.uri}:${r.range.start.line + 1}`).join("\n")}`
                  : `No definition found for "${sym}".`);
              })();
            } else { sysMsg(`Finding: ${sym}`); }
            break;
          }
          case "references": {
            const sym = lspSub[1];
            if (!sym) { sysMsg("Usage: /lsp references <symbol>"); break; }
            if (runtime) {
              void (async () => {
                const lsp = runtime.getLspManager() as unknown as { findSymbol(n: string): Promise<readonly { uri: string; range: { start: { line: number; character: number } } }[]>; findReferences(u: string, p: { line: number; character: number }): Promise<readonly { uri: string; range: { start: { line: number } } }[]> };
                const symbols = await lsp.findSymbol(sym);
                if (symbols.length === 0) { appendSystemMessage(`No symbol found: "${sym}"`); return; }
                const first = symbols[0]!;
                const refs = await lsp.findReferences(first.uri, first.range.start);
                appendSystemMessage(refs.length > 0
                  ? `${refs.length} references to "${sym}":\n${refs.slice(0, 15).map((r) => `  ${r.uri}:${r.range.start.line + 1}`).join("\n")}${refs.length > 15 ? `\n  ... and ${refs.length - 15} more` : ""}`
          ...
          case "rename": {
            ...
            if (!oldSym || !newSymName) { sysMsg("Usage: /lsp rename <old> <new>"); break; }
            if (runtime) {
              void (async () => {
                const lsp = runtime.getLspManager() as unknown as { findSymbol(n: string): Promise<readonly { uri: string; range: { start: { line: number; character: number } } }[]>; rename(u: string, p: { line: number; character: number }, n: string): Promise<{ filesAffected: number; editsApplied: number }> };
                const symbols = await lsp.findSymbol(oldSym);
                if (symbols.length === 0) { appendSystemMessage(`No symbol found: "${oldSym}"`); return; }
                const first = symbols[0]!;
            ...
            } else { sysMsg(`Rename: ${oldSym} → ${newSymName}`); }
            break;
          }
          case "hover": {
            const target = lspSub.slice(1).join(" ");
            if (!target) { sysMsg("Usage: /lsp hover <file:line:col>"); break; }
            if (runtime) {
              void (async () => {
            ...
            } else { sysMsg(`Getting hover info for: ${target}`); }
            break;
          }
          default:
      ...
      case "/inbox": {
        if (!runtime) { sysMsg("Dispatch inbox requires WotannRuntime."); return true; }
        ...
        const taskLines = allTasks.slice(0, 15).map((t: DispatchTask) =>
        ...
        sysMsg([
          ...
          "├──────────────────────────────────────────────────────────┤",
          ...(taskLines.length > 0 ? taskLines : ["│  (empty — no tasks in inbox)                              "]),
          allTasks.length > 15 ? `│  ... and ${allTasks.length - 15} more tasks                                 ` : "",
          "╰──────────────────────────────────────────────────────────╯",
        ].filter(Boolean).join("\n"));
        return true;
      }
      ...
      case "/channels": {
        if (!runtime) { sysMsg("Channel health requires WotannRuntime."); return true; }
        ...
        const channelLines = healthData.map((h: ChannelHealth) => {
          ...
          const uptime = h.upSince > 0
            ? `${((Date.now() - h.upSince) / 60000).toFixed(0)}m`
            : "—";
        ...
        sysMsg([
          ...
          "├──────────────────────────────────────────────────────────┤",
          ...(channelLines.length > 0 ? channelLines : ["│  (no channels registered)                                 "]),
          "╰──────────────────────────────────────────────────────────╯",
        ].join("\n"));
        return true;
      }
      ...
      case "/dispatch": {
        if (!runtime) { sysMsg("Dispatch management requires WotannRuntime."); return true; }
      ...
      case "/deeplink": {
        ...
        const ctx: DeepLinkContext = {
          workingDir,
          setMode: (mode) => { if (runtime) runtime.setMode(mode as WotannMode); setCurrentMode(mode as WotannMode); },
      ...
      case "/roe": {
        if (!runtime) { sysMsg("Rules of Engagement requires WotannRuntime."); return true; }
        const roe = runtime.getRulesOfEngagement();
        const roeSub = (parts[1] ?? "").toLowerCase();
        const roeArg = parts.slice(2).join(" ").trim();

        if (roeSub === "start") {
          const validTypes: readonly ROESessionType[] = ["security-research", "ethical-hacking", "ctf", "pentest"];
          ...
          const { scope } = session;
          const isEmpty = scope.domains.length === 0 && scope.ipRanges.length === 0 &&
            scope.pathPatterns.length === 0 && scope.excludedTargets.length === 0;
          sysMsg([
          ...
          const lines = trail.slice(-15).map((entry) =>
      ...
      case "/enhance": {
        ...
        if (!runtime) { sysMsg("Prompt enhancer requires WotannRuntime."); return true; }
      ...
      case "/research": {
        ...
        if (!runtime) { sysMsg("Research requires WotannRuntime."); return true; }
      ...
      case "/search": {
        ...
        if (!runtime) { sysMsg("Search requires WotannRuntime."); return true; }
        ...
        if (searchResults.length === 0) {
          ...
          const resultLines = searchResults.slice(0, 10).map((r, i) =>
      ...
      case "/health": {
        ...
        void (async () => {
          try {
            ...
            const grade = report.healthScore >= 80 ? "A" : report.healthScore >= 60 ? "B" : report.healthScore >= 40 ? "C" : "D";
            appendSystemMessage([
              ...
              "",
              report.largestFiles.length > 0 ? `  Largest files:\n${report.largestFiles.slice(0, 5).map(f => `    ${f.path} (${f.lineCount} lines)`).join("\n")}` : "",
            ].filter(Boolean).join("\n"));
          } catch (error) {
      ...
      case "/timeline": {
        if (!runtime) { sysMsg("Timeline requires WotannRuntime."); return true; }
        ...
        void (async () => {
          try {
            ...
            if (summary.entryCount === 0) {
              ...
              const categories = summary.categories.map((c: { readonly category: string; readonly count: number }) => `    ${c.category}: ${c.count}`).join("\n");
              appendSystemMessage([
                "Temporal Memory Timeline",
                "",
      ...
      case "/actions": {
        if (messages.length === 0) { sysMsg("No messages to act on."); return true; }
  ...
  }, [exit, providers, stats, currentMode, currentModel, turnCount, history, runtime, getSkillRegistry, themeName, workingDir, promptValue, isIncognito, messages]);
  ...
  const handleSubmit = useCallback(async (input: string) => {
    ...
    if (attachmentResolution.errors.length > 0) {
      setMessages((prev) => [...prev, ...attachmentResolution.errors.map((error) => ({
      ...
      try {
        ...
        if (!abort.signal.aborted) {
          ...
          for (const err of errors) {
            setMessages((prev) => [...prev, { role: "system" as const, content: `Error: ${err}` }]);
          }
  ...
  }, [currentModel, initialProvider, runtime, messages, handleSlashCommand, syncStatsFromRuntime, workingDir, appendSystemMessage]);
  ...
  const contextSources: readonly ContextSource[] = React.useMemo(() => {
    ...
    if (budget.systemPromptTokens > 0) sources.push({ name: "System prompt", tokens: budget.systemPromptTokens, type: "system" });
    if (budget.memoryTokens > 0) sources.push({ name: "Memory", tokens: budget.memoryTokens, type: "memory" });
    if (budget.toolSchemaTokens > 0) sources.push({ name: "Tool schemas", tokens: budget.toolSchemaTokens, type: "tools" });
    if (budget.conversationTokens > 0) sources.push({ name: "Conversation", tokens: budget.conversationTokens, type: "conversation" });
    return sources;
  ...
  return (
    <Box flexDirection="column" height="100%">
      {showStartup && (
        <StartupScreen version={version} providers={providers} />
      )}
      ...
      <Box flexGrow={1}>
        ...
        >
          <Text bold color={currentTheme.colors.primary}>
            {activePanel === "diff" ? "Diff Panel" : activePanel === "agents" ? "Agent Tree" : "Task View"}
          </Text>
            ...
            <Box flexDirection="column" gap={1}>
              ...
              {history.slice(0, 5).map((entry, index) => (
                <Text key={`task-${index}`} dimColor>
                  {index + 1}. {entry.slice(0, 36)}{entry.length > 36 ? "..." : ""}
                </Text>
              ))}
            </Box>
          )}
      ...
      {showMessageActions && messages.length > 0 && (() => {
      ...
      {isIncognito && (
        <Box paddingX={1}>
          <Text color="yellow" bold>INCOGNITO</Text>
