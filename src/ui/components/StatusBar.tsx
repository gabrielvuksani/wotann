...
function modeColor(mode: string): string {
  switch (mode) {
    ...
    case "autonomous": return "magenta";
    case "bypass": return "red";
    case "guardrails-off": return "red";
...
export function StatusBar({
  ...
  return (
    <Box borderStyle="single" borderColor={isStreaming ? "green" : "gray"} paddingX={1} justifyContent="space-between">
      {/* Left: model + provider */}
      <Box gap={1}>
        {isStreaming && <Text color="green">●</Text>}
        <Text color="cyan" bold>{model.replace("claude-", "").replace("gpt-", "")}</Text>
      ...
      <Box gap={1}>
        <Text color={modeColor(mode)} bold>{mode}</Text>
        {mode === "guardrails-off" && roeSessionActive && (
          <Text color="yellow" bold>[ROE]</Text>
        )}
        {mode === "guardrails-off" && !roeSessionActive && (
          <Text color="red" bold>[NO-ROE]</Text>
        )}
      ...
      <Box gap={1}>
        ...
        <Text color={contextColor(contextPercent)}>
          {contextBar(contextPercent)}
        </Text>
        <Text dimColor>{contextPercent}%</Text>
        <Text dimColor>R{reads} E{edits} B{bashCalls}</Text>
      </Box>
    </Box>
  );
}
