...
export class ScheduleStore {
   ...
   * conceptual v0→v1 (initialize() already created v1 tables — no-op),
   * then stores user_version=1.
   */
  private migrateLegacy(): void {
    ...
    const tx = this.db.transaction(() => {
      for (let v = current; v < SCHEMA_VERSION; v++) {
        switch (v) {
          case 0:
            // v0 → v1: initial schema already in place. This branch exists
            // so future v1 → v2 migrations can chain (e.g. ALTER TABLE for
            // a new column would land here once SCHEMA_VERSION bumps to 2).
            break;
        }
      }
