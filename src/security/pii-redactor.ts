...
const DEFAULT_CONFIG: PIIRedactorConfig = {
  ...
  categories: new Set([
    ...
    "auth_url", "date_of_birth", "passport",
  ]),
  whitelistPatterns: [],
...
const PII_PATTERNS: readonly PIIPattern[] = [
  ...
  },
  {
    ...
    confidence: 0.85,
    validate: (match) => match !== "127.0.0.1" && match !== "0.0.0.0" && !match.startsWith("192.168."),
    format: (_match, style) => {
  ...
  {
    category: "auth_url",
    pattern: /https?:\/\/[^\s]*(?:token|key|secret|password|auth|credential|apikey|access_token|bearer)[=:][^\s&"']+/gi,
    confidence: 0.90,
    format: (_match, style) => {
  ...
  {
    category: "name_field",
    pattern: /(?:"(?:first_?name|last_?name|full_?name|patient_?name|customer_?name)"\s*:\s*")((?:[^"\\]|\\.)*)"/gi,
    confidence: 0.85,
    format: (match, style) => {
