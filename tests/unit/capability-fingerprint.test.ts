...
describe("Capability Fingerprinter", () => {
  it("returns static capabilities for known models", () => {
    ...
    const result = fp.getFingerprint("anthropic", "claude-opus-4-6");
    ...
    expect(fp.hasCapability("anthropic", "claude-opus-4-6", "tool-calling")).toBe(true);
    expect(fp.hasCapability("anthropic", "claude-opus-4-6", "vision")).toBe(true);
    expect(fp.hasCapability("anthropic", "claude-opus-4-6", "extended-thinking")).toBe(true);
    expect(fp.hasCapability("anthropic", "claude-opus-4-6", "computer-use")).toBe(true);
  });
  ...
  it("bestModelForCapability finds the right model", () => {
    const fp = new CapabilityFingerprinter();
    // Prime the cache
    fp.getFingerprint("anthropic", "claude-opus-4-6");
  ...
  it("caches fingerprints", () => {
    const fp = new CapabilityFingerprinter();
    const first = fp.getFingerprint("anthropic", "claude-opus-4-6");
    const second = fp.getFingerprint("anthropic", "claude-opus-4-6");
